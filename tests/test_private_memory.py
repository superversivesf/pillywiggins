...
@pytest.fixture
def memory():
    return PrivateMemory(database_url="postgresql://test:test@localhost:5432/testdb", agent_id="puck")
...
async def test_search_returns_results(memory):
    ...
    mock_row.items = lambda: {"id": "abc-123", "content": "remembered thing", "metadata": {"key": "value"}, "created_at": datetime(2026, 1, 1, tzinfo=timezone.utc), "similarity": 0.95}.items()
